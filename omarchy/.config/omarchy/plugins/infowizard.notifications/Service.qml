...
Item {
  ...
  readonly property int criticalPopupDuration: 10000
  readonly property int maxPopupDuration: 30000

  function durationFor(urgency, expireTimeout) {
    switch (urgency) {
    case NotificationUrgency.Critical:
      // Upstream Omarchy never expires critical toasts (returns 0), which
      // means every requireInteraction web notification (Teams, WhatsApp,
      // Slack, ...) piles up until clicked. Give them a long but finite
      // timeout instead so they auto-dismiss like everything else.
